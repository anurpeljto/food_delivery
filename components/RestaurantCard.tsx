...
<TouchableWithoutFeedback onPress={() => navigation.navigate("Restaurant", {item})}>
      <View className="mr-6 bg-white rounded-3xl shadow-lg" style={{ shadowColor: "gray", shadowRadius: 7 }}>
        <Image className="h-36 w-64 rounded-t-3xl" source={item.image} />
        <View className="px-1 pb-4 space-y-2">
          <Text className="text-lg font-bold pt-2 pl-0">{item.name}</Text>
          <View className="flex-row items-center space-x-1">
            <Icon.Star className="h-4 w-4" fill="yellow" stroke="orange" />
          ...
          <View className="flex-row items-center space-x-1">
            <Icon.MapPin color="red" width="15" height="15" />
